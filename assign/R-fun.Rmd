...
3.1 以上代码运行无误以后，把**`R-fun-自己名字.R`**的代码，通过`github desktop`同步到自己github账户下的**code**文件夹；

3.2 根据**`R-fun-自己名字.R`**的代码，新建一个同名的`.Rmd`文件（R Markdown格式的文件），可通过AI辅助（推荐使用`VS code + github coploit`的组合，当然使用`Deepseek`等工具也可以），运行成功后保存为一个新的**`html格式`**文件，这是R代码运行后的结果展示，这个文件保存后，也同样同步到`github`的**code文**件夹；

3.3 以上R Markdown格式的文件运行后，生成**`html格式`**文件后，点击右上角的`publish`，选择在`Rpub`公布（需要新建一个账号），保存链接地址，粘贴在`github`名下`README文件`，保存后同步到`github`。
